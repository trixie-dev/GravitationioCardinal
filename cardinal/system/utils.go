...
func queryTargetPlayerBalance(world cardinal.WorldContext, targetNickname string) (types.EntityID, *comp.Balance, error) {
	var playerID types.EntityID
	var playerBalance *comp.Balance
	var err error
	searchErr := cardinal.NewSearch().Entity(
		filter.Exact(filter.Component[comp.Player](), filter.Component[comp.Balance](), filter.Component[comp.Health]())).Each(world,
		func(id types.EntityID) bool {
			var player *comp.Player
			player, err = cardinal.GetComponent[comp.Player](world, id)
			if err != nil {
	...
	return playerID, playerBalance, err
}

func queryTargetPlayerHealth(world cardinal.WorldContext, targetNickname string) (types.EntityID, *comp.Health, error) {
	var playerID types.EntityID
	var playerHealth *comp.Health
	var err error
	searchErr := cardinal.NewSearch().Entity(
